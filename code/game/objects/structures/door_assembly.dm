...
/obj/structure/door_assembly/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/pen))
		...
		if(mineral)
			...
			user.visible_message(span_notice("[user] welds the [mineral] plating off the airlock assembly."), span_notice("I start to weld the [mineral] plating off the airlock assembly..."))
			if(W.use_tool(src, user, 40, volume=50))
				to_chat(user, span_notice("I weld the [mineral] plating off."))
				new mineral_path(loc, 2)
				var/obj/structure/door_assembly/PA = new previous_assembly(loc)
				transfer_assembly_vars(src, PA)

		else if(glass)
			user.visible_message(span_notice("[user] welds the glass panel out of the airlock assembly."), span_notice("I start to weld the glass panel out of the airlock assembly..."))
			if(W.use_tool(src, user, 40, volume=50))
				to_chat(user, span_notice("I weld the glass panel out."))
			...
			user.visible_message(span_warning("[user] disassembles the airlock assembly."), \
								span_notice("I start to disassemble the airlock assembly..."))
			if(W.use_tool(src, user, 40, volume=50))
				to_chat(user, span_notice("I disassemble the airlock assembly."))
				deconstruct(TRUE)
		...
		if(!anchored )
			...
			if(door_check)
				user.visible_message(span_notice("[user] secures the airlock assembly to the floor."), \
									 span_notice("I start to secure the airlock assembly to the floor..."), \
									 span_hear("I hear wrenching."))

				if(W.use_tool(src, user, 40, volume=100))
					if(anchored)
						return
					to_chat(user, span_notice("I secure the airlock assembly."))
			...
			user.visible_message(span_notice("[user] unsecures the airlock assembly from the floor."), \
								 span_notice("I start to unsecure the airlock assembly from the floor..."), \
								 span_hear("I hear wrenching."))
			if(W.use_tool(src, user, 40, volume=100))
				if(!anchored)
					return
				to_chat(user, span_notice("I unsecure the airlock assembly."))
				name = "airlock assembly"
				setAnchored(FALSE)
		...
		user.visible_message(span_notice("[user] wires the airlock assembly."), \
							span_notice("I start to wire the airlock assembly..."))
		if(W.use_tool(src, user, 40, amount=1))
			if(state != AIRLOCK_ASSEMBLY_NEEDS_WIRES)
				return
			state = AIRLOCK_ASSEMBLY_NEEDS_ELECTRONICS
			to_chat(user, span_notice("I wire the airlock assembly."))
		...
		user.visible_message(span_notice("[user] cuts the wires from the airlock assembly."), \
							span_notice("I start to cut the wires from the airlock assembly..."))

		if(W.use_tool(src, user, 40, volume=100))
			if(state != AIRLOCK_ASSEMBLY_NEEDS_ELECTRONICS)
				return
			to_chat(user, span_notice("I cut the wires from the airlock assembly."))
					...
					if(!glass)
						if(istype(G, /obj/item/stack/sheet/rglass) || istype(G, /obj/item/stack/sheet/glass))
							...
							user.visible_message(span_notice("[user] adds [G.name] to the airlock assembly."), \
												span_notice("I start to install [G.name] into the airlock assembly..."))
							if(do_after(user, 40, target = src))
								if(G.get_amount() < 1 || glass)
									return
								if(G.type == /obj/item/stack/sheet/rglass)
									to_chat(user, span_notice("I install [G.name] windows into the airlock assembly."))
									...
									to_chat(user, span_notice("I install regular glass windows into the airlock assembly."))
							...
							if(G.get_amount() >= 2)
								...
								if(do_after(user, 40, target = src))
									if(G.get_amount() < 2 || mineral)
										return
									to_chat(user, span_notice("I install [M] plating into the airlock assembly."))
									G.use(2)
									var/mineralassembly = text2path("/obj/structure/door_assembly/door_assembly_[M]")
									var/obj/structure/door_assembly/MA = new mineralassembly(loc)
									transfer_assembly_vars(src, MA, TRUE)
							else
								to_chat(user, span_warning("I need at least two sheets add a mineral cover!"))
					else
						to_chat(user, span_warning("I cannot add [G] to [src]!"))
				else
					to_chat(user, span_warning("I cannot add [G] to [src]!"))
		...
		user.visible_message(span_notice("[user] finishes the airlock."), \
							 span_notice("I start finishing the airlock..."))

		if(W.use_tool(src, user, 40, volume=100))
			if(loc && state == AIRLOCK_ASSEMBLY_NEEDS_SCREWDRIVER)
				to_chat(user, span_notice("I finish the airlock."))
