...
//	if(get_area(source) == GLOB.areas_by_type[/area/centcom/wizard_station])     No such turf exists.
//		return
	...
	var/mob/living/worm = new /mob/living/simple_animal/wizard_worm(get_turf(source))
	source.mind?.transfer_to(worm)
	source.forceMove(worm)

/mob/living/simple_animal/wizard_worm
	name = "Magic Worm"
...
/mob/living/simple_animal/wizard_worm/has_gravity(turf/gravity_turf)
	return TRUE

/mob/living/simple_animal/wizard_worm/can_be_pulled()
	return FALSE

/mob/living/simple_animal/wizard_worm/Initialize(mapload, spawn_bodyparts = TRUE)
	. = ..()
//	AddElement(/datum/element/wall_tearer)  No need for this. 

	if(spawn_bodyparts)
		build_tail()

/mob/living/simple_animal/wizard_worm/proc/build_tail(mob/living/tail)
	AddComponent(/datum/component/mob_chain, vary_icon_state = TRUE)
	var/mob/living/simple_animal/wizard_worm/prev = src
	for(var/i in 1 to 5)
		prev = new_segment(behind = prev)
	update_appearance(UPDATE_ICON_STATE)

/mob/living/simple_animal/wizard_worm/proc/new_segment(mob/living/simple_animal/wizard_worm/behind)
	var/mob/living/segment = new type(drop_location(), FALSE)
	// ADD_TRAIT(segment, TRAIT_PERMANENTLY_MORTAL, INNATE_TRAIT) No wizard antag. 
